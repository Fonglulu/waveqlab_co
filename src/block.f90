...
contains
  ...
  subroutine init_block(mesh_source, type_of_mesh,material_source,response,fd_type, interpol, &
    ...
    use material, only : init_material, init_material_from_file
    ...
    if (mesh_source == 'file') then
       call init_grid_from_file(B%id, B%G, btp%aqrs, btp%bqrs)
    else if (mesh_source == 'compute') then
       if (type_of_mesh == 'cartesian') then
          call init_grid_cartesian(B%G, btp%aqrs, btp%bqrs)
          ...
          call init_grid_curve(B%G, btp%aqrs, btp%bqrs, &
               ...
               btp%topography_type, btp%topography_path, .false., use_topography, topo, ny, nz)
       end if
    end if

    ! Initialize materials properties, either from file or based on problem type
    if (material_source == 'file') then
        call init_material_from_file(B%id, B%M, B%G,btp%material_path)
    else
        call init_material(B%M,B%G,B%I,B%physics,problem, btp%rho_s_p)
    end if
    ...
    B%fd_type =  fd_type
    if (fd_type .eq. 'upwind') then
      B%tau0 =  1.0_wp/0.3486111111111111_wp
    ...
    print *, 'penalty parameter ', B%tau0, fd_type, B%fd_type
  ...
  subroutine partition_block(B, method, periodic, size_in, block_comm)
    ...
    nb = 3
    nF = 9
    call decompose3d(B%G%C, B%I%nq, B%I%nr, B%I%ns, nb, &
