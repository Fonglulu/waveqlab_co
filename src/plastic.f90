...
contains
  ...
  subroutine update_fields_plastic(F, P, G, M, dt, t,problem,response)
    ...
    character(*),intent(in) :: response,problem                         ! response and problem types
    ...
    do k=iz,fz
       do j=iy,fy
          do i=ix,fx
             ...
             !call plastic_flow(F%F(i,j,k,4:9),mu,beta,eta,Gel,Kel,s0,G%X(i,j,k,2),problem,dt)
             !call plastic_flow2(F%F(i,j,k,4:9),s0,G%X(i,j,k,2),problem,dt)
             !
          end do
       end do
    end do
