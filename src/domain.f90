...
contains
  ...
  subroutine init_domain(D, infile)
    ...
    if (in_fault_comm(1)) call init_fault_output(D%name, D%fault, D%B(1)%G%C, fault_comms(1))
    if (in_fault_comm(2) .and. .not.in_fault_comm(1)) call init_fault_output(D%name, D%fault, D%B(2)%G%C, fault_comms(2))
